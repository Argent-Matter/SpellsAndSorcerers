...
public class WandItem extends Item {
    ...
    public static final Component JOINER_LAST = Component.translatable("tooltip.motm.joiner.last");
    ...
    public ItemStack getDefaultInstance() {
        return MOTMUtil.createWand(MOTMWandAbilities.DUMMY.get(), null);
    }

    @Override
    public Component getName(ItemStack stack) {
        WandComponent component = stack.get(MOTMDataComponents.WAND);
        if (component == null) {
            return super.getName(stack).copy().append(Component.translatable("ability.motm.dummy"));
        }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        WandComponent component = itemstack.get(MOTMDataComponents.WAND);
        if (component == null) {
            return reference;
        }
    ...
    public boolean deductManaFromUser(LivingEntity user, ItemStack stack, int timeCharged) {
        if (user instanceof Player player && player.isCreative())
            return true;
        if (user.hasData(MOTMAttachmentTypes.MANA) && MOTMConfig.Server.useMana.get()) {
            var manaCap = user.getData(MOTMAttachmentTypes.MANA);
            var manaToDeduct = 1 + timeCharged / 4 * (6 - 0 /* stack.getEnchantmentLevel(MOTMEnchantments.MANA_EFFICIENCY) */);
