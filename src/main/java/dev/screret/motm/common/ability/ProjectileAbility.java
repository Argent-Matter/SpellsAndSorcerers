...
public abstract class ProjectileAbility<T extends ProjectileAbility<T>> extends WandAbility<T> {
    ...
    public ProjectileAbility(int useDuration, int cooldownDuration, float damagePerHit, boolean applyEnchants,
    ...
    public ProjectileAbility(int useDuration, int cooldownDuration, float damagePerHit, boolean applyEnchants, int color,
                             int distance) {
        super(useDuration, cooldownDuration, damagePerHit, applyEnchants,
                new DustParticleOptions(new Vector3f(Vec3.fromRGB24(color).toVector3f()), 2.0F), color);
        // oldParticle = new BlockParticleOption(ParticleTypes.BLOCK_MARKER, Blocks.BARRIER.defaultBlockState())
        this.distance = distance;
    }
    ...
    public static <
            W extends ProjectileAbility<W>> Products.P7<RecordCodecBuilder.Mu<W>, Integer, Integer, Float, Boolean, ParticleOptions, Integer, Integer> projectileCodecStart(RecordCodecBuilder.Instance<W> instance) {
