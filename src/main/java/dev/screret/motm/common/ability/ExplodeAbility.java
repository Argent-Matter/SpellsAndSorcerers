...
public class ExplodeAbility extends SubAbility<ExplodeAbility> {
    ...
    public static final DustParticleOptions PARTICLE = new DustParticleOptions(
            new Vector3f(Vec3.fromRGB24(0xFFAA0000).toVector3f()), 2.0F);
    public static final Vec3 RANDOM_DEVIATION = new Vec3(0.125D, 0.125D, 0.125D);

    private static final MapCodec<ExplodeAbility> CODEC = RecordCodecBuilder
            .mapCodec(instance -> WandAbility.codecStart(instance).apply(instance, ExplodeAbility::new));

    public ExplodeAbility() {
        super(100, 60, 1, true, ExplodeAbility.PARTICLE, 0xAA0000, EnumSet.of(HitFlags.BLOCK, HitFlags.NONE));
    }

    public ExplodeAbility(int useDuration, int cooldownDuration, float damagePerHit, boolean applyEnchants,
                          ParticleOptions particle, int color) {
        super(useDuration, cooldownDuration, damagePerHit, applyEnchants, particle, color,
                EnumSet.of(HitFlags.BLOCK, HitFlags.NONE));
    }
