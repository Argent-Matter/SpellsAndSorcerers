...
public class ShootAbility extends WandAbility {
    ...
    public InteractionResultHolder<ItemStack> execute(Level level, LivingEntity user, ItemStack stack, WandAbilityInstance.WrappedVec3 currentPosition, int timeCharged) {
        if (!level.isClientSide) {
            ...
            if (hitResult != null) {
                if (particle != null)
                    MITMUtil.spawnParticlesInLine(level, userPos, hitResult.getLocation(), particle, ((int) userPos.distanceTo(hitResult.getLocation()) * 4), randomDeviation, false);
            ...
            if (particle != null)
                MITMUtil.spawnParticlesInLine(level, userPos, blockHit.getLocation(), particle, (int) (user.position().distanceTo(blockHit.getLocation()) * 4), randomDeviation, false);
        }
        return InteractionResultHolder.pass(stack);
    }
}